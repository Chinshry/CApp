package com.chinshry.application.view
...
class CustomTabView(context: Context, attrs: AttributeSet? = null) : LinearLayout(context, attrs) {
    ...
    class Tab(
        ...
        @ColorInt var textColorSelect: Int = ColorUtils.getColor(R.color.blue),
        ...
        fun setTextColor(@ColorInt normal: Int?, @ColorInt select: Int?): Tab {
            this.textColorNormal = normal ?: ColorUtils.getColor(R.color.grey)
            this.textColorSelect = select ?: ColorUtils.getColor(R.color.blue)
            return this
        }
    ...
    fun addTab(tab: Tab) {
        val view: View = LayoutInflater.from(context).inflate(R.layout.custom_tab_item_layout, null)
