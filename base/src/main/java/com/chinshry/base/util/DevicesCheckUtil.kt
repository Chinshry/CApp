...
object DevicesCheckUtil {
    ...
    private fun checkFeaturesByHardware(): CheckItemResult {
        ...
        val hardware = getProperty("ro.hardware")?.lowercase(Locale.ROOT) ?: return CheckItemResult(checkItemName, RESULT_MAYBE_EMULATOR)
    ...
    private fun checkFeaturesByFlavor(): CheckItemResult {
        ...
        val result = if (emulatorFlavorList.any { flavor.lowercase(Locale.ROOT).contains(it) }) RESULT_EMULATOR else RESULT_UNKNOWN
    ...
    private fun checkFeaturesByModel(): CheckItemResult {
        ...
        val result = if (emulatorModelList.any { model.lowercase(Locale.ROOT).contains(it) }) RESULT_EMULATOR else RESULT_UNKNOWN
    ...
    private fun checkFeaturesByManufacturer(): CheckItemResult {
        ...
        val result =
            when {
                manufacturer.lowercase(Locale.ROOT).contains("genymotion") -> RESULT_EMULATOR
                manufacturer.lowercase(Locale.ROOT).contains("netease") -> RESULT_EMULATOR //网易MUMU模拟器
                else -> RESULT_UNKNOWN
            }
    ...
    private fun checkFeaturesByBoard(): CheckItemResult {
        ...
        val result = if (emulatorBoardList.any { board.lowercase(Locale.ROOT).contains(it) }) RESULT_EMULATOR else RESULT_UNKNOWN
    ...
    private fun checkFeaturesByPlatform(): CheckItemResult {
        ...
        val result = if (platform.lowercase(Locale.ROOT).contains("android")) RESULT_EMULATOR else RESULT_UNKNOWN
    ...
    private fun readCpuInfo(): String {
        ...
        try {
            ...
            result = sb.toString().lowercase(Locale.ROOT)
    ...
    private fun getSimulatorBrand(context: Context, resultPkg: MutableList<CheckItemResult>): String {
        ...
        val knownSimulator = SimulatorDetectorUtil.SimulatorList.filter { emulatorPkgName.contains(it.code)}
        ...
        when {
            ...
            emulatorPkgName.contains("duoyi") -> {
                ...
                return SimulatorDetectorUtil.UNKNOWN_SIMULATOR.name
            }
        }
    }
